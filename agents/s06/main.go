...
var TOOLS = []anthropic.ToolUnionParam{
	...
	{OfTool: &anthropic.ToolParam{
		Name: "compact", Description: anthropic.String("Trigger manual conversation compression."),
		InputSchema: anthropic.ToolInputSchemaParam{
			Properties: map[string]any{"focus": map[string]any{"type": "string", "description": "What to preserve in the summary"}},
		},
	}},
}
...
var TOOL_HANDLERS = map[string]ToolHandler{
	...
	"load_skill": runLoadSkill, "compact": runCompact,
}
...
func runCompact(input map[string]any) string {
...
func microCompact(messages *[]anthropic.MessageParam) {
	usedToolMap := make(map[string]string)
	for _, msg := range *messages {
		...
		for _, block := range msg.Content {
			if block.OfToolUse != nil {
				usedToolMap[block.OfToolUse.ID] = block.OfToolUse.Name
			}
		}
	}
	...
	for i := range *messages {
		...
		for j := range msg.Content {
			...
			if block.OfToolResult != nil {
				refs = append(refs, toolResultRef{block: block.OfToolResult})
			}
		}
	}
	...
	toClear := refs[:len(refs)-KEEP_RECENT]
	for _, ref := range toClear {
		name := usedToolMap[ref.block.ToolUseID]
		if name == "" {
			name = "unknown"
		}
		if name == "read_file" {
		...
		ref.block.Content = []anthropic.ToolResultBlockParamContentUnion{
			{OfText: &anthropic.TextBlockParam{Text: fmt.Sprintf("[Previous: used %s]", name)}},
		}
	}
}
...
func autoCompact(messages *[]anthropic.MessageParam) {
	if _, err := os.Stat("transcripts"); os.IsNotExist(err) {
		os.Mkdir("transcripts", 0755)
	}
	transcriptsPath := fmt.Sprintf("transcripts/session_%d.jsonl", time.Now().Unix())
	file, err := os.OpenFile(transcriptsPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		logrus.Error(err)
		return
	}
	...
	historyBytes, _ := json.Marshal(*messages)
	historyStr := string(historyBytes)
	if len(historyStr) > 80000 {
		historyStr = historyStr[len(historyStr)-80000:]
	}

	resp, err := client.Messages.New(context.Background(), anthropic.MessageNewParams{
		Model: MODEL, MaxTokens: 2000,
		Messages: []anthropic.MessageParam{
			anthropic.NewUserMessage(anthropic.NewTextBlock("Summarize this conversation for continuity:\n" + historyStr)),
		},
	})
	if err != nil {
		logrus.Error(err)
	...
	*messages = []anthropic.MessageParam{
		anthropic.NewUserMessage(anthropic.NewTextBlock(fmt.Sprintf("[Compressed]\n\n%s", summary))),
	}
}
...
func main() {
	...
	SYSTEM_PROMPT = fmt.Sprintf("You are a coding agent at %s. Use tools to solve tasks.", WorkingDir)
	...
	for {
		...
		resp, loopErr := agentLoop(&messages)
		if loopErr != nil {
...
func agentLoop(messages *[]anthropic.MessageParam) (*anthropic.Message, error) {
	for {
		microCompact(messages)

		fmt.Printf("\033[33mTotal Tokens: %d\033[0m\n", estimateTokens(messages))
