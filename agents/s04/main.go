...
var SUBAGENT_SYSTEM_PROMPT = fmt.Sprintf("You are a coding subagent at %s. Complete the given task, then summarize your findings.", WorkingDir)
...
var PARENT_TOOLS = append(CHILD_TOOLS, anthropic.ToolUnionParam{
	OfTool: &anthropic.ToolParam{
		Name: "task", Description: anthropic.String("Spawn a subagent with fresh context. It shares the filesystem but not conversation history."),
		InputSchema: anthropic.ToolInputSchemaParam{
...
func runSubagent(prompt string) string {
	...
	var lastResp *anthropic.Message
	for i := 0; i < 30; i++ {
		resp, err := client.Messages.New(context.Background(), anthropic.MessageNewParams{
			Model: MODEL, MaxTokens: 8000,
			System:   []anthropic.TextBlockParam{{Text: SUBAGENT_SYSTEM_PROMPT}},
			Tools:    CHILD_TOOLS,
			Messages: messages,
		})
		if err != nil {
			return "subagent request failed: " + err.Error()
		}
		lastResp = resp
		...
		for _, b := range resp.Content {
			...
			if err := json.Unmarshal(b.Input, &input); err != nil {
				tr = append(tr, anthropic.NewToolResultBlock(b.ID, truncate("Error: invalid tool input: "+err.Error()), false))
				continue
			}
			handler := TOOL_HANDLERS[b.Name]
			var output string
			if handler == nil {
			...
			tr = append(tr, anthropic.NewToolResultBlock(b.ID, truncate(output), false))
		}
	...
	for {
		...
		*messages = append(*messages, anthropic.NewAssistantMessage(ab...))
		...
		for _, b := range resp.Content {
			...
			var input map[string]any
			if err := json.Unmarshal(b.Input, &input); err != nil {
				tr = append(tr, anthropic.NewToolResultBlock(b.ID, fmt.Sprintf("Error: decode tool input for %s: %v", b.Name, err), false))
				continue
			}
			var output string
			if b.Name == "task" {
