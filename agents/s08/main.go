...
var TOOLS = []anthropic.ToolUnionParam{
	...
	{OfTool: &anthropic.ToolParam{
		Name:        "background_run",
		Description: anthropic.String("Run command in background thread. Returns task_id immediately"),
		InputSchema: anthropic.ToolInputSchemaParam{
			Properties: map[string]any{
				"command": map[string]any{"type": "string"},
			},
...
var bgManager *background.BackgroundManager
...
var TOOL_HANDLERS = map[string]ToolHandler{
	...
	"write_file":       runWrite,
	"edit_file":        runEdit,
	"background_run":   runBackgroundRun,
	"check_background": runCheckBackground,
}
...
func main() {
	...
	bgManager = background.NewBackgroundManager()
...
func runBackgroundRun(input map[string]any) string {
...
func runCheckBackground(input map[string]any) string {
	taskID, ok := input["task_id"].(string)
	if !ok {
		taskID = ""
	}
	return bgManager.Check(taskID)
}
